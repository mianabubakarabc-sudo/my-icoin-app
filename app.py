...
# Dynamic Tables Re-Creation & Version Management
try:
...
def get_or_create_user(email):
    ...
    if row:
        ...
        default_bal = 1250.00
        cursor.execute("INSERT INTO users (email, balance) VALUES (?, ?)", (email, default_bal))
        conn.commit()
        return default_bal
...
# ==============================================================================
    ...
    if st.session_state.connected_gmail is None:
        ...
        with col_bl2:
            ...
            if st.button("🔗 CONNECT BINANCE IDENTITY GATEWAY", use_container_width=True, key="connect_bridge_submit"):
                if "@" in input_gmail and "." in input_gmail:
                    st.session_state.connected_gmail = input_gmail.strip()
                    st.success("⚡ Identity Vault Bridged Successfully!")
                    time.sleep(1)
                    st.rerun()
                else:
        ...
        # Refresh loop control to keep price ticker real-time
        time.sleep(2.5)
        st.rerun()