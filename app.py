...
def get_or_create_user(email, pin_code=None):
    ...
    cursor.execute("SELECT balance FROM users WHERE LOWER(email)=?", (email_clean,))
    row = cursor.fetchone()
    if row:
        return row[0]
    else:
        ...
        cursor.execute("INSERT INTO users (email, balance, pin) VALUES (?, ?, ?)", (email_clean, default_bal, pin_code))
        conn.commit()
        return default_bal

def update_user_balance(email, amount):
    cursor.execute("UPDATE users SET balance = balance + ? WHERE LOWER(email)=?", (amount, email.strip().lower()))
    conn.commit()
...
if is_admin_route:
    ...
    if admin_password != "admin123":
        ...
        # SUB-SECTION 3: 👥 SYSTEM REGISTRY & CLIENT ASSET BALANCES (NEW USER DESK)
        st.subheader("👥 REGIONAL USER REGISTRY & LEDGER MANAGEMENT")
        cursor.execute("SELECT id, email, balance, pin FROM users")
        all_registered_users = cursor.fetchall()
        
        if not all_registered_users:
            st.info("No registered users found in the network database.")
        else:
            user_table = "<table class='admin-table'><tr><th>User ID</th><th>Customer Email</th><th>Secure PIN</th><th>USDT Balance</th><th>Est. I-COIN Holdings</th></tr>"
            for u in all_registered_users:
                u_id, u_email, u_balance, u_pin = u
                est_icoins = u_balance / current_market_base if current_market_base > 0 else 0
                user_table += f"<tr><td>{u_id}</td><td style='color:#FFD700; font-weight:bold;'>{u_email}</td><td><code>{u_pin}</code></td><td style='color:#14F195;'>${u_balance:,.2f}</td><td>{est_icoins:.5f} I-COIN</td></tr>"
            user_table += "</table>"
            st.markdown(user_table, unsafe_allow_html=True)
            
            # Form to modify user balance/pin directly from Admin Panel
            st.markdown("##### 🛠️ Quick Balance & PIN Adjustment Desk")
            selected_user_email = st.selectbox("Select Client Email to Modify:", [u[1] for u in all_registered_users], key="admin_select_user_edit")
            
            cursor.execute("SELECT balance, pin FROM users WHERE email=?", (selected_user_email,))
            u_row = cursor.fetchone()
            if u_row:
                curr_bal, curr_pin = u_row
                col_u1, col_u2 = st.columns(2)
                with col_u1:
                    adj_balance = st.number_input("Adjust USDT Balance:", value=float(curr_bal), step=50.0, key="adj_bal_input")
                with col_u2:
                    adj_pin = st.text_input("Modify/Reset 4-6 Digit PIN:", value=str(curr_pin), max_chars=6, key="adj_pin_input")
                
                if st.button("💾 APPLY REGISTRY CHANGES", use_container_width=True, key="save_user_adj_btn"):
                    cursor.execute("UPDATE users SET balance=?, pin=? WHERE email=?", (adj_balance, adj_pin.strip(), selected_user_email))
                    conn.commit()
                    st.success(f"Successfully updated ledger settings for {selected_user_email}!")
                    time.sleep(1)
                    st.rerun()
    ...
    if st.session_state.connected_gmail is None:
        ...
        with col_bl2:
            st.markdown("""
                ...
                <h3 style='text-align: center; color: #FFD700; font-weight: 800; letter-spacing: 3px; margin-top: 5px; margin-bottom: 25px; font-family: sans-serif;'>I-COIN GLOBAL</h3>
            """, unsafe_allow_html=True)
            
            # Professional Navigation Tabs for Log In & Sign Up Choice
            gateway_choice = st.radio("Portal Access Option:", ["Existing User (Log In)", "New User (Sign Up)"], horizontal=True, key="portal_tab_choice")
            st.write("")

            # --- A. LOG IN INTERFACE ---
            if gateway_choice == "Existing User (Log In)":
                ...
                if st.button("🔗 CONNECT SECURE IDENTITY GATEWAY", use_container_width=True, key="connect_bridge_submit"):
                    email_clean = input_gmail.strip().lower()
                    ...
                    if "@" in email_clean and "." in email_clean:
                        if len(pin_clean) >= 4 and len(pin_clean) <= 6 and pin_clean.isdigit():
                            ...
                            cursor.execute("SELECT pin FROM users WHERE LOWER(email)=?", (email_clean,))
                            ...
                            if user_db_row:
                                ...
                                st.error("❌ Account not found! Please switch to 'Sign Up' tab to register your email first.")
                        else:
            ...
            # --- B. SIGN UP INTERFACE (WITH TWO-TIME PASSWORD & CAPTCHA) ---
            else:
                signup_gmail = st.text_input(
                    "Register Binance Gmail / Account ID:", 
                    placeholder="yourname@gmail.com", 
                    key="binance_gmail_signup"
                )
                signup_pin = st.text_input(
                    "Create Secure 4-6 Digit Account PIN:", 
                    placeholder="Create Password (Numbers only)", 
                    type="password",
                    max_chars=6,
                    key="binance_pin_signup"
                )
                signup_confirm_pin = st.text_input(
                    "Confirm Your Account PIN:", 
                    placeholder="Retype Password to Verify", 
                    type="password",
                    max_chars=6,
                    key="binance_pin_confirm_signup"
                )
                
                # Anti-Robot Captcha Check Engine
                st.markdown("<p style='color:#FFD700; font-size:12px; margin-bottom:2px; font-weight:bold;'>🤖 ANTI-ROBOT VERIFICATION SYSTEM</p>", unsafe_allow_html=True)
                correct_answer = st.session_state.num1 + st.session_state.num2
                user_captcha = st.number_input(f"Solve Security Lock: {st.session_state.num1} + {st.session_state.num2} = ?", step=1, value=0, key="captcha_input_field")
                
                st.markdown('<div class="action-btn" style="margin-top:15px;">', unsafe_allow_html=True)
                if st.button("🔐 INITIALIZE NEW SECURE ACCOUNT", use_container_width=True, key="signup_bridge_submit"):
                    email_clean = signup_gmail.strip().lower()
                    pin_clean = signup_pin.strip()
                    confirm_clean = signup_confirm_pin.strip()
                    
                    if "@" in email_clean and "." in email_clean:
                        if len(pin_clean) >= 4 and len(pin_clean) <= 6 and pin_clean.isdigit():
                            if pin_clean != confirm_clean:
                                st.error("❌ PIN MISMATCH! The verification password does not match the created password.")
                            elif user_captcha != correct_answer:
                                st.error("❌ CAPTCHA VERIFICATION FAILED! Prove you are human to authorize database entry.")
                            else:
                                # 🛑 CRITICAL CASE-INSENSITIVE CHECK: Check if user email is already registered
                                cursor.execute("SELECT pin FROM users WHERE LOWER(email)=?", (email_clean,))
                                if cursor.fetchone():
                                    st.error("❌ Email already registered under matching logs! Please switch to 'Log In'.")
                                else:
                                    # Auto-Register New User with their chosen PIN instantly
                                    get_or_create_user(email_clean, pin_clean)
                                    st.session_state.connected_gmail = email_clean
                                    st.success("🎉 Welcome! New Identity Securely Initialized with $1250.00 USDT Starting Balance.")
                                    # Reset captcha for next cycle
                                    st.session_state.num1 = random.randint(1, 10)
                                    st.session_state.num2 = random.randint(5, 15)
                                    time.sleep(1.5)
                                    st.rerun()
